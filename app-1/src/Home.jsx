...
function Home({ action, onScrollToProject }) {
  return (
    <div className='relative h-screen w-full overflow-hidden bg-[#0a0a0a] text-white'>
      
      {/* 1. Main Content Layer */}
      <div className={`clip-circle flex h-full w-full flex-col md:flex-row ${action ? 'clip-circle-open' : ''}`}>

        {/* Left Section: Introduction */}
        <div className='flex h-full w-full flex-col justify-center bg-gradient-to-br from-gray-950 via-neutral-900 to-gray-950 px-8 md:w-1/2 md:px-16 xl:px-24'>
          <div className={action ? 'animate-reveal [animation-delay:0.5s]' : 'opacity-0'}>
            <h1 className='mb-6 text-5xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-stone-100 to-stone-500 md:text-6xl lg:text-7xl xl:text-8xl'>
              Welcome.
            </h1>
            ...
            <div className='mt-10 flex gap-6'>
              {/* Button triggers the scroll function from props */}
              <button 
                onClick={onScrollToProject}
                className='group relative overflow-hidden rounded-full border border-stone-700 px-8 py-3 transition-all hover:border-stone-200 active:scale-95'
              >
        ...
        <div className='relative flex h-full w-full flex-col items-center justify-center bg-stone-900 md:w-1/2'>
          ...
          <div className={`flex flex-col items-center transform transition-all duration-1000 delay-1000 ${action ? "animate-[open_0.4s_linear_forwards]" : "opacity-0"}`}>
            
            <div
              className="group relative mb-8 h-64 w-64 overflow-hidden rounded-2xl border border-white/10 shadow-2xl transition-all duration-500 hover:scale-[1.02] hover:border-stone-500 xl:h-96 xl:w-80"
              style={{
